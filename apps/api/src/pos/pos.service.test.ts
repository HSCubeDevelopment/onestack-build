  ...
  const tx = {
    sale: {
      ...
      findFirst: async ({ where, include }: any) => {
        ...
        if (!r) return null;
        return include?.lines ? { ...r, lines: lines.filter((l) => l.saleId === r.id) } : r;
      },
      findMany: async () =>
        sales.map((s) => ({ ...s, lines: lines.filter((l) => l.saleId === s.id) })),
    ...
    saleLine: {
      create: async ({ data }: any) => {
        const r = { id: `l${lines.length + 1}`, createdAt: new Date(), ...data };
        lines.push(r);
        return r;
      },
      findMany: async ({ where }: any) => lines.filter((l) => l.saleId === where.saleId),
      deleteMany: async ({ where }: any) => {
        const i = lines.findIndex((l) => l.id === where.id);
...
describe('PosService', () => {
  it('opens a sale, adds lines with GST, completes with a tender', async () => {
    ...
    let s = await svc.addLine('t1', sale.id, {
      description: 'Wash',
      quantity: 2,
      unitPriceCents: 5000,
    });
  ...
  it('will not complete an empty sale or edit a completed one', async () => {
    ...
    await expect(
      svc.addLine('t1', sale.id, { description: 'x', quantity: 1, unitPriceCents: 1 }),
