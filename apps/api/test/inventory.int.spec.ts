...
describe.skipIf(!hasDb)('Inventory & stock (Phase 4)', () => {
  ...
  it('creates an item and lists it', async () => {
    itemId = (
      await http()
        ...
        .set(auth(a))
        .send({ name: 'Brake pad', quantityOnHand: 3, reorderLevel: 5, unit: 'each' })
        .expect(201)
    ).body.id;
  ...
  it('receives + uses stock, updating on-hand', async () => {
    const recv = (
      await http()
        .post(`/api/v1/inventory/${itemId}/movement`)
        .set(auth(a))
        .send({ delta: 10, reason: 'receive' })
        .expect(201)
    ).body;
  ...
  it('low-stock filter returns only items at/under reorder', async () => {
    await http()
      .post('/api/v1/inventory')
      .set(auth(a))
      .send({ name: 'Wiper', quantityOnHand: 0, reorderLevel: 2 })
      .expect(201);
