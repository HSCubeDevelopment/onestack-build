...
describe.skipIf(!hasDb)('Waitlist & auto-fill (Phase 4)', () => {
  ...
  beforeAll(async () => {
    ...
    bayId = (await http().post('/api/v1/resources').set(auth(a)).send({ type: 'bay', name: 'Bay 1' }).expect(201)).body.id;
  });

  afterAll(async () => {
    await app.close();
    for (const t of [a.tenantId, b.tenantId]) {
      for (const tbl of ['onestack_waitlist_entry', 'onestack_booking', 'onestack_resource', 'onestack_feature_flag']) {
  ...
  it('adds to the waitlist and lists who is waiting', async () => {
    entryId = (
      await http().post('/api/v1/waitlist').set(auth(a)).send({ name: 'Jane', phone: '0400000000', resourceId: bayId }).expect(201)
    ).body.id;
  ...
  it('finds candidates for a freed resource slot', async () => {
    const cands = (await http().get(`/api/v1/waitlist?resourceId=${bayId}`).set(auth(a)).expect(200)).body;
  ...
  it('fills the slot: creates a booking and marks the entry booked', async () => {
    const res = (
      await http()
        ...
        .set(auth(a))
        .send({ resourceId: bayId, startsAt: '2027-02-10T09:00:00.000Z', endsAt: '2027-02-10T10:00:00.000Z' })
        .expect(201)
    ).body;
  ...
  it("is tenant-isolated: shop B sees none of A's waitlist and can't fill A's entry", async () => {
    ...
    await http()
      .post(`/api/v1/waitlist/${entryId}/fill`)
      .set(auth(b))
      .send({ resourceId: bayId, startsAt: '2027-02-11T09:00:00.000Z', endsAt: '2027-02-11T10:00:00.000Z' })
      .expect(404);
  });
});
