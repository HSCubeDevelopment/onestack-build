...
describe.skipIf(!hasDb)('Point of sale (Phase 4)', () => {
  ...
  beforeAll(async () => {
    admin = adminPrisma();
    a = await makeTenant(admin, 'POS A');
    b = await makeTenant(admin, 'POS B');
    app = await createApp();
    await app.init();
  });
  afterAll(async () => {
    ...
    for (const t of [a.tenantId, b.tenantId])
      for (const tbl of ['onestack_sale_line', 'onestack_sale'])
  ...
  it('rings up a walk-in sale with GST and completes it', async () => {
    ...
    await http()
      ...
      .set(auth(a))
      .send({ description: 'Air freshener', quantity: 3, unitPriceCents: 500 })
      .expect(201);
    const s = (
      await http()
        .post(`/api/v1/sales/${saleId}/lines`)
        .set(auth(a))
        .send({ description: 'Wash', quantity: 1, unitPriceCents: 4000 })
        .expect(201)
    ).body;
