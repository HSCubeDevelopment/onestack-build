...
describe.skipIf(!hasDb)('Automotive extensions — reorder + stocktake (Phase 4)', () => {
  ...
  beforeAll(async () => {
    admin = adminPrisma();
    a = await makeTenant(admin, 'Par A');
    b = await makeTenant(admin, 'Par B');
    app = await createApp();
    await app.init();
  });
  afterAll(async () => {
    ...
    for (const t of [a.tenantId, b.tenantId])
      for (const tbl of ['onestack_stock_movement', 'onestack_inventory_item'])
  ...
  it('suggests reorder up to par for low items', async () => {
    itemId = (
      await http()
        ...
        .set(auth(a))
        .send({ name: 'Clip', quantityOnHand: 2, reorderLevel: 5, parLevel: 20 })
        .expect(201)
    ).body.id;
    await http()
      .post('/api/v1/inventory')
      .set(auth(a))
      .send({ name: 'Bracket', quantityOnHand: 40, reorderLevel: 5, parLevel: 20 })
      .expect(201);
  ...
  it('stocktake sets the counted on-hand', async () => {
    const after = (
      await http()
        .post(`/api/v1/inventory/${itemId}/stocktake`)
        .set(auth(a))
    ...
    expect(
      (await http().get('/api/v1/inventory/reorder').set(auth(a)).expect(200)).body,
    ).toHaveLength(0);
    await http()
      .post(`/api/v1/inventory/${itemId}/stocktake`)
      .set(auth(a))
