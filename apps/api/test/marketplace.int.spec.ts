...
describe.skipIf(!hasDb)('Integration marketplace (Phase 4)', () => {
  ...
  beforeAll(async () => {
    admin = adminPrisma();
    a = await makeTenant(admin, 'Mkt A');
    b = await makeTenant(admin, 'Mkt B');
    app = await createApp();
    await app.init();
  });
  afterAll(async () => {
    ...
    for (const t of [a.tenantId, b.tenantId])
      await admin.$executeRawUnsafe(
        `DELETE FROM "onestack_integration_connection" WHERE "tenantId" = $1::uuid`,
        t,
      );
  ...
  it('disconnects and rejects an unknown slug', async () => {
    const v = (await http().post('/api/v1/integrations/xero/disconnect').set(auth(a)).expect(201))
      .body;
