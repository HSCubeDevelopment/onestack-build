...
describe.skipIf(!hasDb)('Referral engine (Phase 4)', () => {
  ...
  beforeAll(async () => {
    admin = adminPrisma();
    a = await makeTenant(admin, 'Ref A');
    b = await makeTenant(admin, 'Ref B');
    app = await createApp();
    await app.init();
    referrerId = (
      await http()
  ...
  afterAll(async () => {
    ...
    for (const t of [a.tenantId, b.tenantId])
      for (const tbl of ['onestack_referral', 'onestack_referral_code', 'onestack_contact'])
  ...
  it('issues a referral code for a customer', async () => {
    const c = (await http().post(`/api/v1/referrals/codes/${referrerId}`).set(auth(a)).expect(201))
      .body;
    expect(c.code).toMatch(/^REF-/);
    const again = (
      await http().post(`/api/v1/referrals/codes/${referrerId}`).set(auth(a)).expect(201)
    ).body;
  ...
  it('records + converts + rewards a referral', async () => {
    refId = (
      await http()
        ...
        .set(auth(a))
        .send({ referrerContactId: referrerId, referredName: 'Bob' })
        .expect(201)
    ).body.id;
    await http().post(`/api/v1/referrals/${refId}/reward`).set(auth(a)).send({}).expect(400); // must convert first
    const conv = (
      await http().post(`/api/v1/referrals/${refId}/convert`).set(auth(a)).send({}).expect(201)
    ).body;
    expect(conv.status).toBe('converted');
    const rew = (
      await http()
        .post(`/api/v1/referrals/${refId}/reward`)
        .set(auth(a))
        .send({ note: '500 pts' })
        .expect(201)
    ).body;
