...
describe.skipIf(!hasDb)('Public API & webhooks (Phase 4)', () => {
  ...
  let server: http.Server; let received: { sig: string; body: string; event: string } | null = null; let url = '';
  const http_ = () => request(app.getHttpServer());
  const auth = (t: TestTenant) => ({ Authorization: `Bearer ${t.ownerToken}` });

  beforeAll(async () => {
    admin = adminPrisma(); a = await makeTenant(admin, 'WH A'); b = await makeTenant(admin, 'WH B');
    ...
    server = http.createServer((req, res) => {
      ...
      req.on('end', () => {
        received = { sig: String(req.headers['x-onestack-signature'] ?? ''), body, event: String(req.headers['x-onestack-event'] ?? '') };
  ...
  afterAll(async () => {
    ...
    for (const t of [a.tenantId, b.tenantId]) for (const tbl of ['onestack_webhook_delivery','onestack_webhook_endpoint'])
  ...
  it('registers an endpoint and delivers a correctly-signed test payload', async () => {
    const ep = (await http_().post('/api/v1/webhooks').set(auth(a)).send({ url, events: ['*'] }).expect(201)).body;
    expect(ep.secret).toMatch(/^whsec_/);
    const delivery = (await http_().post(`/api/v1/webhooks/${ep.id}/test`).set(auth(a)).expect(201)).body;
    ...
    // Delivery is logged.
    expect((await http_().get(`/api/v1/webhooks/${ep.id}/deliveries`).set(auth(a)).expect(200)).body.length).toBeGreaterThanOrEqual(1);
  });

  it('records a failed delivery for an unreachable URL', async () => {
    const ep = (await http_().post('/api/v1/webhooks').set(auth(a)).send({ url: 'http://127.0.0.1:1/dead' }).expect(201)).body;
    const delivery = (await http_().post(`/api/v1/webhooks/${ep.id}/test`).set(auth(a)).expect(201)).body;
