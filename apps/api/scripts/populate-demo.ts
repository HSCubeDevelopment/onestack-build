...
async function main() {
  ...
  // Insurer
  const insurer = await call('POST', '/contacts', {
    displayName: 'AAMI Insurance',
    phone: '0132244',
  });
  ...
  for (const c of customers) {
    ...
    const vehicle = await call('POST', `/contacts/${contact.id}/vehicles`, {
      rego: c.rego,
      make: c.make,
      model: c.model,
      year: c.year,
    ...
    const job = await call('POST', '/work-items', {
      ...
      fields: {
        ...
        description: `${c.make} ${c.model} — repair`,
        ...(claim ? { claim } : {}),
      },
    });
    ...
    await call('POST', `/quotes/${quote.id}/lines`, {
      description: 'Panel labour',
      type: 'labour',
      quantity: 2,
      unitPriceCents: 9500,
    });
    ...
    // A note
    await call('POST', `/work-items/${job.id}/notes`, {
      body: 'Customer dropped the car off; keys in lockbox.',
    });
    ...
    for (const ev of path[c.target] ?? [])
      await call('POST', `/work-items/${job.id}/transition`, { event: ev });
    ...
    if (c.name === 'Jane Motorist') {
      ...
      await call('POST', `/invoices/${inv.id}/excess-split`, {
        primaryPayerContactId: insurer.id,
        primaryAmountCents: authorised,
        excessAmountCents: excessCents,
        excessPayerContactId: contact.id,
      });
      const full = await call('GET', `/invoices/${inv.id}`);
      const excess = full.portions.find((p: any) => p.payerContactId === contact.id);
      await call('POST', `/invoices/${inv.id}/payments`, {
  ...
  await call('POST', `/public/lead-forms/${form.publicToken}/submit`, {
    name: 'Sam Carter',
    phone: '0400999888',
    message: 'Scratched my rear door, need a quote',
  ...
  await call('POST', `/public/lead-forms/${form.publicToken}/submit`, {
    ...
    message: 'Hail damage on the bonnet',
  });
