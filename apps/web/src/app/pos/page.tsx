  ...
  return (
    <>
      <PageHead title="Point of sale" sub="In-person checkout for walk-ins">
        {!active ? (
          <button className="btn primary" onClick={openSale}>
            + New sale
          </button>
        ) : null}
      </PageHead>
...
function Register({
  ...
  const addLine = async () => {
    ...
    try {
      onChange(
        await api.post<Sale>(`/sales/${sale.id}/lines`, {
  ...
  const removeLine = async (lineId: string) =>
    onChange(await api.del<Sale>(`/sales/${sale.id}/lines/${lineId}`));
  const complete = async (tenderType: 'cash' | 'card' | 'other') => {
    setBusy(true);
    setError(null);
    try {
      await api.post(`/sales/${sale.id}/complete`, { tenderType });
      onDone();
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setBusy(false);
  ...
  return (
    <div className="card">
      <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button className="btn" onClick={voidSale}>
          Void
        </button>
      </div>
      ...
      <div style={{ borderTop: '1px solid var(--border, #e2e7eb)', marginTop: 14, paddingTop: 12 }}>
        ...
        <div className="row" style={{ justifyContent: 'space-between' }}>
          ...
          <span>{money(sale.gstCents)}</span>
        </div>
        <div className="row" style={{ justifyContent: 'space-between', marginTop: 4 }}>
          <strong>Total</strong>
          <strong style={{ fontSize: 20 }}>{money(sale.totalCents)}</strong>
        </div>
      </div>
      ...
      <div className="row" style={{ gap: 8, marginTop: 14 }}>
        <button
          ...
          disabled={busy || sale.lines.length === 0}
        ...
        <button
          className="btn"
          disabled={busy || sale.lines.length === 0}
          onClick={() => complete('other')}
        >
          Other
        </button>
      </div>
      <p className="muted" style={{ fontSize: 12, marginTop: 8 }}>
        Records how they paid — card processing comes with the payments phase.
      </p>
    </div>
  );
}
